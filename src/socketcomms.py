...
def get_command(tamp1, args1, tamp2, args2):
    ...
    if tamp1 == "LowThroughput":
        command = f"{command} rate {args1[0]}kbit latency {args1[2]}ms"
    if tamp2 == "LowThroughput":
        command = f"{command} rate {args2[0]}kbit latency {args2[2]}ms"
    return command
...
def process_input(data):
    tamp_ind = data.find("),Packet_cons")
    tamp_input = data[37:tamp_ind]
    if tamp_input.startswith('None'):
        sep = tamp_input.find(',')
    else:
        sep = tamp_input.find(')')+1
    tamp1 = tamp_input[:sep]
    tamp2 = tamp_input[sep+1:]
